    ...
    public void testVoidRequest() throws Exception
    {
        DataService mock = new MockUp<DataService>() {
            ...
            public void executeInterceptors(IntuitMessage intuitMessage) {
                assertTrue(intuitMessage.getRequestElements().getRequestParameters().containsKey(RequestElements.REQ_PARAM_INCLUDE));
                assertTrue(intuitMessage.getRequestElements().getRequestParameters().containsValue(OperationType.VOID.toString()));
            }
        }.getMockInstance();
